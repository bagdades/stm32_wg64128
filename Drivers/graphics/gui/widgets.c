...
tFont *default_font = (tFont*)&Arial7;
...
void newWidget(widget_t **new, widgetType type, struct screen_t *scr){
	...
	switch(type){
		...
		case widget_bmp_button:
			...
			break;
		case widget_bmp:
			w->content = malloc(sizeof(bmp_widget_t));
			break;
		default:
			Error_Handler();
	}
	...
	switch(w->type){
		...
		case widget_editable:
			...
			if(dis->type == field_string){
				...
				val_ui = strsum(dis->displayString);
				break;
			}
			else{
...
void widgetClearField(widget_t* w){
	...
	button_widget_t* button;
	bmp_widget_t* bmp;
	uint8_t cHeight = ug_GetFont(&ug)->chars->image->height;
	/* uint8_t r; */
	...
	if(w->frameType!=frame_combo){
		...
		if(w->parent->refresh < screen_Erase ){
			switch((uint8_t)w->type){
				case widget_bmp:
					bmp = (bmp_widget_t*)w->content;
					ug_FillRectangle(&ug, w->posX, w->posY, bmp->xbm->width, bmp->xbm->height);
					break;

				case widget_bmp_button:
					button = (button_widget_t*)w->content;
					ug_FillRectangle(&ug, w->posX, w->posY, button->xbm[0]+4, button->xbm[1]+4);
					break;

				case widget_button:
					button = (button_widget_t*)w->content;
					ug_FillRectangle(&ug, w->posX, w->posY, w->width, cHeight+1);
					break;

				case widget_display:
					ug_FillRectangle(&ug, w->posX, w->posY, w->width, cHeight+1);       // Draw black square to erase previous data
					break;

				case widget_editable:
				case widget_multi_option:
					ug_FillRectangle(&ug, w->posX - 2 ,w->posY, w->width, cHeight+1);
					break;
			}
		}
		if( (sel && (sel->state==widget_edit) && (dis->type!=field_string) && (w->frameType==frame_auto)) || (w->frameType==frame_solid) ){
			...
			/* } */
			if(sel){
				ug_FillRectangle(&ug, w->posX - 2, w->posY, w->width - 3, cHeight + 2);
			}
			else{
				ug_FillRectangle(&ug, w->posX - 2 ,w->posY, w->width + 2, cHeight + 2);
			}
...
void widgetAlign(widget_t* w){
	...
	switch(w->type){
		case widget_button:
			button = (button_widget_t *)w->content;
			strWidth = ug_GetStrWidth(&ug, ug_GetFont(&ug), button->displayString);
			break;

		case widget_display:
		case widget_editable:
			if(dis->type == field_string){
				/* strWidth = ug_GetStrWidth(&ug, ug_GetFont(&ug), dis->getData()); */
				strWidth = ug_GetStrWidth(&ug, ug_GetFont(&ug), dis->displayString);
			}
			else{
				strWidth = ug_GetStrWidth(&ug, ug_GetFont(&ug), displayString);
			}
			break;

		case widget_multi_option:
			strWidth = ug_GetStrWidth(&ug, ug_GetFont(&ug), (char*)edit->options[*(uint8_t*)dis->getData()]);
			break;
		default:
			return;
	}
	...
	if(sel && (w->frameType!=frame_disabled)){      // If selectable, extra space for not overlapping the frame
		if(w->width < strWidth+7){                    // If width too small
			if(strWidth+7<ug_GetXDim(&ug)){                   // If fits oled size
	...
	switch(dispAlign){
		case align_disabled:
			break;
		case align_center:
			w->posX =(ug_GetXDim(&ug) - w->width)/2;
			break;
		case align_right:
			w->posX = ug_GetXDim(&ug) - w->width;
			break;
		case align_left:
		default:
			w->posX = 0;
...
uint8_t default_widgetDraw(widget_t *w) {
	...
	if( dis ){
		if(ug_GetFont(&ug) != dis->font){
			ug_SetFont(&ug, dis->font);
		}
		cHeight = ug_GetFont(&ug)->chars->image->height;
	}
	else if((w->type == widget_button)||(w->type == widget_bmp_button)){
		button = (button_widget_t*)w->content;
		if(w->type == widget_button){
			if(ug_GetFont(&ug) != button->font){
				ug_SetFont(&ug, button->font);
			}
			cHeight = ug_GetFont(&ug)->chars->image->height;
		}
	}
	...
	if(refresh){
		...
		switch(w->type){
			...
			case widget_bmp:
				/* ucg_DrawBmp(w->posX, w->posY, bmp->xbm, w->fcolor, w->bcolor); */
			...
			case widget_button:
				ug_DrawString(&ug, button->stringStart, w->posY + 2, button->displayString);
				break;
			...
			case widget_display:
				if(dis->type == field_string){
					/* ucg_SetForeColor(&ucg, w->fcolor); */
					ug_SetMode(&ug, LCD_MODE_NORM);
					ug_DrawString(&ug, dis->stringStart, w->posY,  dis->displayString);
					/* ug_DrawString(&ug, dis->stringStart, w->posY,  dis->getData()); */
				}
				else{
					/* ucg_SetForeColor(&ucg, w->fcolor); */
					ug_SetMode(&ug, LCD_MODE_NORM);
					...
					/* ucg_SetForeColor(&ucg, tcolor); */
				break;

			case widget_editable:
				if(dis->type == field_string){
					ug_DrawString(&ug,  dis->stringStart, w->posY+2,  dis->getData());
					if(sel->state == widget_edit){
						char t[20];
						strcpy(t,dis->getData());

						t[edit->current_edit+1]=0;
						uint8_t x2=ug_GetStrWidth(&ug, ug_GetFont(&ug), t);

						t[edit->current_edit]=0;
						uint8_t x1=ug_GetStrWidth(&ug, ug_GetFont(&ug), t);

						/* ucg_SetForeColor(&ucg, C_BLACK); */
						ug_SetMode(&ug, LCD_MODE_INVERSE);
						ug_FillRectangle(&ug, dis->stringStart, w->posY+ cHeight, w->width, 2);

						/* ucg_SetForeColor(&ucg, w->fcolor); */
						ug_SetMode(&ug, LCD_MODE_NORM);
						ug_FillRectangle(&ug,  dis->stringStart+x1, w->posY+ cHeight, x2-x1, 2);
					...
					ug_DrawString(&ug,w->posX, w->posY,  dis->displayString);
					/* ucg_WriteString(&ucg,dis->stringStart, w->posY+2,  dis->displayString); */
				}
				break;

			case widget_multi_option:
				ug_DrawString(&ug, dis->stringStart, w->posY+2, edit->options[*(uint8_t*)dis->getData()]);// Draw string
				break;
	...
	if(frameDraw ||(refresh && w->frameType==frame_outline)){
		switch(w->type){
			case widget_bmp_button:
				/* ucg_SetForeColor(&ucg, frameColor); */
				if(frameColor)
					ug_SetMode(&ug, LCD_MODE_NORM);
				else
					ug_SetMode(&ug, LCD_MODE_INVERSE);
				ug_DrawFrame(&ug, w->posX, w->posY, button->xbm[0]+4,  button->xbm[1]+4);
				break;
			case widget_editable:
			case widget_button:
			case widget_multi_option:
				...
				/* 	r = w->radius; */
				/* } */
				ug_DrawFrame(&ug, w->posX - 2 ,w->posY - 1, w->width - 4, cHeight+2);
			default:
				break;
		}
...
uint8_t comboBoxDraw(widget_t *w) {
	...
	uint16_t yDim = ug_GetYDim(&ug) - w->posY;
	uint8_t height;
	int8_t frameY=0;
	int8_t posY;
	bool drawFrame=1;
	...
	if(ug_GetFont(&ug) != combo->font){
		ug_SetFont(&ug, combo->font);
	}
	height= ug_GetFont(&ug)->chars->image->height + 2;                 // +1 to allow separation between combobox items
	...
	for(uint8_t y = 0; y < yDim / height; ++y) {
		...
		/* ucg_SetForeColor(&ucg, C_BLACK); */
		ug_SetMode(&ug, LCD_MODE_NORM);
		ug_FillRectangle(&ug, 0, y * height + w->posY, ug_GetXDim(&ug), height + 1);
		/* ucg_SetForeColor(&ucg, default_color); */
		ug_SetMode(&ug, LCD_MODE_NORM);
		if ((item->type==combo_Screen)||(item->type==combo_Action)){      // If screen or action item, just draw the label
			...
			if(item->dispAlign==align_left){
				ug_DrawString(&ug, 4, y * height + w->posY +1, item->text);
			}
			else{
				uint8_t len = ug_GetStrWidth(&ug, ug_GetFont(&ug), item->text);
				if(item->dispAlign==align_right){
					ug_DrawString(&ug, ug_GetXDim(&ug)-3-len, y * height + w->posY +1, item->text);
				}
				else{
					ug_DrawString(&ug, (ug_GetXDim(&ug)-1-len)/2, y * height + w->posY +1, item->text);
				}
			}
		}
			...
			if(sel->state==widget_edit){                                      // If edit mode
				...
				/* ucg_SetForeColor(&ucg, w->fcolor); */
				ug_SetMode(&ug, LCD_MODE_INVERSE);
				ug_FillRectangle(&ug, 0, frameY, ug_GetXDim(&ug), height);
				/* ucg_SetForeColor(&ucg, C_BLACK); */
				/* ucg_SetBackColor(&ucg, w->fcolor); */
				ug_SetMode(&ug, LCD_MODE_INVERSE);
			}
			...
			if(item->type==combo_MultiOption){
				len = ug_GetStrWidth(&ug, ug_GetFont(&ug), edit->options[*(uint8_t*)dis->getData()]);
			}
			else if(item->type==combo_Editable){
				if(dis->type==field_int32){
					int32_t val_ui = *(int32_t*)dis->getData();                         // Get data
					...
					dis->displayString[dis->reservedChars]=0;                           // Ensure last string char is 0
					len=ug_GetStrWidth(&ug, ug_GetFont(&ug), dis->displayString);
				}
					...
					len=ug_GetStrWidth(&ug, ug_GetFont(&ug), displayString);
				}
			}

			posY = y * height + w->posY;                                          // Set widget Ypos same as the current combo option
			dis->stringStart = ug_GetXDim(&ug)-len-5;                                   // Align to the left measuring actual string width
			if(item->type==combo_Editable){
				if((dis->type==field_string && (sel->state==widget_edit))){
					char str[sizeof(displayString)+1];
					uint8_t start,width;
					strcpy(str,displayString);
					str[edit->current_edit+1]=0;
					width=ug_GetStrWidth(&ug, ug_GetFont(&ug), str);
					str[edit->current_edit]=0;
					start=ug_GetStrWidth(&ug, ug_GetFont(&ug), str);
					width-=start;
					...
					ug_DrawString(&ug,dis->stringStart, posY+1, dis->displayString);
				}
			}
			else if(item->type==combo_MultiOption){
				ug_DrawString(&ug,dis->stringStart, posY+1,  edit->options[*(uint32_t*)dis->getData()]);
			}
			ug_DrawString(&ug, 4, y * height + w->posY +1, item->text);          // Draw the combo item label
		}
		do {
			item = item->next_item;
	...
	if(drawFrame){
		/* ucg_SetForeColor(&ucg, w->fcolor); */
		ug_SetMode(&ug, LCD_MODE_NORM);
		ug_DrawFrame(&ug, 0, frameY, ug_GetXDim(&ug) - 1, height);
	}
	return 1;
}
#endif
...
int comboBoxProcessInput(widget_t *w, RE_Rotation_t input, RE_State_t *state) {
	...
	uint16_t yDim = ug_GetYDim(&ug) - w->posY;
	uint16_t height = ug_GetFont(&ug)->chars->image->height + 2;
...
int default_widgetProcessInput(widget_t *w, RE_Rotation_t input, RE_State_t *state){
	...
	if(input == Click) {
		switch (sel->state) {
			...
			case widget_edit:
				...
				else if(dis->type == field_time || dis->type == field_date) {
					edit->current_edit += 3;
	...
	if( edit && sel->state==widget_edit) {
		...
		else if(dis->type==field_int32 || dis->type == field_temp){
